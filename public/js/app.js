...
(function() {
  ...
  function toggleTheme() {
    ...
    // 重新渲染拼接方案以适配主题（Vue 组件内处理）
    if (window._stitch && window._stitch.rerenderTheme) window._stitch.rerenderTheme();
  }
  ...
  var i18n = {
    zh: {
      ...
      homeDesc4: '13 个 ID 产品系列，按系列浏览全部主机及关联配件。',
      ...
      bomThIdx: '#', bomThType: '类型', bomThName: '物料名称', bomThImg: '主图', bomThDesc: '描述', bomThCode: '物料代码', bomThRemark: '备注', bomThAction: '操作',
      ...
      stNoPlanLead: '已完成参数分析，当前无法生成可用方案，原因如下：',
      stNoPlanSuggestionLabel: '建议：',
      stNoPlanDistanceTitle: '工作距离不适配', stNoPlanDistanceDetail: '输入的工作距离为 {distance} mm，不在任一可计算型号的支持范围内。', stNoPlanDistanceSuggestion: '请调整安装距离，使其落入产品支持的工作距离范围。',
      stNoPlanPpmLowTitle: 'PPM 低于识读要求', stNoPlanPpmLowDetail: '可达到的最高 PPM 为 {value}，低于当前码制要求的 {limit}。', stNoPlanPpmLowSuggestion: '可增大模块尺寸、缩短工作距离，或选用更高分辨率/更长焦距型号。',
      stNoPlanPpmHighTitle: 'PPM 高于计算范围', stNoPlanPpmHighDetail: '可达到的最低 PPM 为 {value}，高于当前码制上限 {limit}。', stNoPlanPpmHighSuggestion: '可增大工作距离、减小模块尺寸，或选用更短焦距型号。',
      stNoPlanCountTitle: '所需相机数量超限', stNoPlanCountDetail: '当前参数下预计至少需要 {count} 台相机，超过系统支持的 32 台上限。', stNoPlanCountSuggestion: '请减小覆盖范围或重叠区域，也可在 PPM 允许时增大工作距离。',
      stNoPlanDataTitle: '产品数据无法计算', stNoPlanDataDetail: '产品数据库未加载，或当前数据中没有完整的焦距与像元参数。', stNoPlanDataSuggestion: '请刷新页面后重试；若问题持续，请检查选型数据文件。',
    ...
    en: {
      ...
      homeDesc4: 'Browse all hosts and associated accessories across 13 ID product series.',
      ...
      bomThIdx: '#', bomThType: 'Type', bomThName: 'Part Name', bomThImg: 'Image', bomThDesc: 'Description', bomThCode: 'Part Code', bomThRemark: 'Remark', bomThAction: 'Action',
      ...
      stNoPlanLead: 'The parameters were analyzed, but no usable plan can be generated for the following reason:',
      stNoPlanSuggestionLabel: 'Suggestion: ',
      stNoPlanDistanceTitle: 'Working distance is unsupported', stNoPlanDistanceDetail: 'The entered working distance is {distance} mm, outside the supported range of every calculable model.', stNoPlanDistanceSuggestion: 'Adjust the mounting distance so it falls within a supported working-distance range.',
      stNoPlanPpmLowTitle: 'PPM is below the reading requirement', stNoPlanPpmLowDetail: 'The highest achievable PPM is {value}, below the required {limit} for the selected code type.', stNoPlanPpmLowSuggestion: 'Increase module size, shorten the working distance, or use a higher-resolution/longer-focal-length model.',
      stNoPlanPpmHighTitle: 'PPM is above the calculation range', stNoPlanPpmHighDetail: 'The lowest achievable PPM is {value}, above the upper limit of {limit} for the selected code type.', stNoPlanPpmHighSuggestion: 'Increase working distance, reduce module size, or use a shorter-focal-length model.',
      stNoPlanCountTitle: 'Required camera count exceeds the limit', stNoPlanCountDetail: 'At least {count} cameras are estimated, exceeding the supported maximum of 32.', stNoPlanCountSuggestion: 'Reduce the coverage or overlap, or increase working distance when the PPM range allows it.',
      stNoPlanDataTitle: 'Product data cannot be calculated', stNoPlanDataDetail: 'The product database is unavailable, or no model has complete focal-length and pixel-size parameters.', stNoPlanDataSuggestion: 'Refresh the page and retry. If the issue persists, check the selection data file.',
