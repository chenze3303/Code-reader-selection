...
海康机器人读码器（Code Reader）智能选型 / 多相机拼接 / 竞品对标 / 配单生成 / 产品对照 / 状态码查询 / 资料下载工具。纯前端实现，无需服务器，浏览器直接打开 `index.html` 即可使用。
...
选定具体型号，输入工作距离和模块尺寸，计算该型号的 FOV、PPM、最大曝光时间。支持按大类→分辨率→型号三级筛选，工作距离范围校验。

#### 核心计算公式

**PPM（Pixels Per Module）计算：**
...
- 模块数 = 视野宽度 / 模块尺寸
- 2D 码（QR）：PPM 4-8 为优秀，>8 过采样，<4 可能无法识别
- 1D 码（Code39）：PPM 1.4-2 为优秀
...
- 传感器尺寸 = 像素数 × 像素尺寸（μm）
- 水平 FOV = WD × (像素数 × 像素尺寸) / 焦距

**综合评分公式：**

```
score = w_resolution × resolution_score + w_ppm × ppm_score + w_distance × distance_score + w_fov × fov_score
```

各分项评分：
- `resolution_score`：分辨率越高越好（对数归一化）
- `ppm_score`：PPM 越接近目标值（2D: 6, 1D: 1.7）越好，使用高斯衰减
- `distance_score`：工作距离与目标距离的匹配度
- `fov_score`：视野覆盖度

**多相机拼接方案：**

当单相机视野不足时，计算所需相机数量和排列方式：

```
所需相机数 = ceil(目标视野 / 单机视野)
重叠区域 = 安全余量 × 单机视野
排列间距 = 单机视野 - 重叠区域
```
...
- **材质**：普通, 高柔, 超柔, 弯头
- 筛选逻辑：同时检查名称和描述字段，支持中英文关键词（HF=高柔, ST=普通）

### 3. 竞品对标（competitor.js）

39 条友商型号与海康对应型号的对标信息，覆盖 7 个品牌：Cognex、Keyence、Datalogic、思谋、华睿、视界、新大陆。

### 4. 产品表（mapping_module.js）

424 条基线型号 ↔ 经销型号的物料代码对照，按系列分组折叠显示。支持资料下载直达海康官网。

### 5. 状态码查询（statuscode_module.js）

224 条海康读码器 SDK 状态码定义，按 10 个分类组织。支持模糊搜索、分类筛选、点击复制。

### 6. 方案解决（page-solutions）

固件下载、SDK 参考文档、技术方案等常用资源快速入口。SDK 参考文档链接到独立页面 `sdk-guide.html`。
...
|--------|------|------|
| 首页 | page-home | 功能概览 |
| 智能选型 | page-selection | PPM 计算 + 型号推荐 |
| 多相机拼接 | page-stitch | 复用选型页面，展开拼接卡片 |
| 竞品对标 | page-competitor | 7 大品牌 39 条对标 |
| 配单表 | page-bom | 三级联动 BOM 生成 |
| 产品表 | page-mapping | 基线↔经销对照 + 资料下载 |
| 状态码查询 | page-statuscode | 224 条 SDK 状态码 |
| 方案解决 | page-solutions | 固件/SDK/技术文档链接 |
